...
<script type="text/ecmascript-6">
  ...
  var _callBsMethods = function (vm, method) {
    ...
    // 循环生成的datetimepicker无法获取到其元素 需要判断DateTimePicker是否存在
    let dateTimePicker = bsElem.data('DateTimePicker');
    let func = null;
    if(dateTimePicker) {
      func = dateTimePicker[method];
    }
  ...
  export default {
    ...
    ready () {
      ...
      //self.$el = el[0];
      if(self.value && typeof self.value === 'string') {
        setCurrent(el, self.value);
      }
